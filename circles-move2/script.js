...
function pulse() {
	(function repeat() {
		d3.selectAll('circle').transition()//.delay(function(d, i){ return i / circleCount * 5000})
		.duration(timeparam)
		.ease('sine')
...
function translateFn() {
	return function(d) {
		return function(t) {
			...
			var t_x = svgWidth / 2 - d.get('x') + drawRadius * Math.cos(t_angle + d.get('angle'));
			var t_y = svgHeight / 2 - d.get('y') + drawRadius * Math.sin(t_angle + d.get('angle'));
